...
export function battleAdviceKeys(state, win = false) {
  if (win || !state?.history) return [];
  const history = state.history,
    playerMoves = history.filter((event) => event.type === 'move-start' && event.side === 'player'),
    damage = history.filter((event) => event.type === 'damage' && event.sourceSide === 'player'),
    tips = [];
  const add = (key) => {
  ...
  if (state.turn >= 5 && !history.some((event) => event.type === 'switch' && event.side === 'player'))
    add('switch');
  if (
    history.filter((event) => event.type === 'status' && event.side === 'player' && event.applied).length >= 4
  )
    add('cleanse');
  if (
    history
      .filter((event) => event.type === 'barrier-hit' && event.side === 'enemy')

...
export const NEGATIVE_STATUSES = Object.freeze(Object.keys(STATUS_DEFINITIONS).filter((id)=>!STATUS_DEFINITIONS[id].positive));
export const POSITIVE_STATUSES = Object.freeze(Object.keys(STATUS_DEFINITIONS).filter((id)=>STATUS_DEFINITIONS[id].positive));

export function hasStatus(combatant, id) { return Boolean(combatant.statuses[id]); }
export function statusStacks(combatant,id){ return combatant.statuses[id]?.stacks || 1; }
...
export function applyStatus(combatant, id, turn, duration=null, stacks=1, sourceCreatureId=null) {
  ...
  combatant.statuses[id]={appliedTurn:turn,stacks:nextStacks,...(duration?{remaining:duration}:{}),...(sourceCreatureId?{sourceCreatureId}:{})};
  return combatant.statuses[id];
}

export function removeStatuses(combatant, ids, count='all') {
  const removed=[];
  for(const id of ids){if(combatant.statuses[id]){delete combatant.statuses[id];removed.push(id);if(count!=='all'&&removed.length>=count)break;}}
  return removed;
}
export function cleanse(combatant,count=1){return removeStatuses(combatant,NEGATIVE_STATUSES,count);}
export function purge(combatant,count=1){return removeStatuses(combatant,POSITIVE_STATUSES,count);}
export function cleanseOne(combatant){return cleanse(combatant,1)[0]||null;}

export function effectiveSpeed(combatant) {
  let multiplier=1;
  if(hasStatus(combatant,'slowed')&&!hasStatus(combatant,'anchored'))multiplier*=.7;
  if(hasStatus(combatant,'drowsy'))multiplier*=.85;
  if(hasStatus(combatant,'stunned'))multiplier*=.55;
  if(hasStatus(combatant,'soaked'))multiplier*=.92;
  if(hasStatus(combatant,'haste'))multiplier*=1.18+.07*(statusStacks(combatant,'haste')-1);

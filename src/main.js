...
if (urlLang === 'en' || urlLang === 'fr') save.language = urlLang;
const i18n = createI18n(save.language);
const { t } = i18n;
...
let gamepadButtons = [],
  ...
  gamepadLoop = 0;
const sound = new SoundSystem(save, () => notify(t('error.audio')));
...
document.body.classList.toggle('reduced-motion', save.reducedMotion);
document.body.classList.toggle('high-contrast', save.highContrast);
persistSave(save);
...
function draftInsightHtml(candidateId) {
  const before = [...(draftRun?.team || [])],
    after = [...before, candidateId],
    oldBonds = teamBonds(before),
    newBonds = teamBonds(after).filter((id) => !oldBonds.includes(id)),
    routeKey = (route) => `${route.setterId}:${route.setupMoveId}:${route.finisherId}:${route.finishMoveId}`,
    oldRoutes = new Set(teamComboRoutes(before).map(routeKey)),
    newRoutes = teamComboRoutes(after).filter((route) => !oldRoutes.has(routeKey(route))),
...
function bindCommon() {
  ...
  if (screen.dataset.page === 'settings' && !screen.querySelector('#high-contrast')) {
    ...
    motion?.insertAdjacentHTML(
      'afterend',
      `<div class="toggle-row contrast-row"><label for="high-contrast"><strong>${t('settings.contrast')}</strong><small>${t('settings.contrastHint')}</small></label><input id="high-contrast" type="checkbox" ${save.highContrast ? 'checked' : ''}></div>`
    );
    screen.querySelector('#high-contrast')?.addEventListener('change', (event) => {
      save.highContrast = event.target.checked;
      persist();
    });
  }
  ...
  if (screen.dataset.page === 'bestiary' && screen.querySelector('.feat-hall .eyebrow'))
    screen.querySelector('.feat-hall .eyebrow').textContent =
      `${save.feats.length}/${Object.keys(FEATS).length}`;
  if (screen.dataset.page === 'bestiary') {
    const emptyRecord = { battles: 0, wins: 0, damage: 0, kos: 0, signatures: 0, assists: 0 },
      favorite = CREATURE_IDS.map((id) => ({ id, ...emptyRecord, ...(save.records?.[id] || {}) })).sort(
        (a, b) =>
          b.battles - a.battles ||
          b.damage - a.damage ||
    ...
    screen.querySelectorAll('.bestiary-card').forEach((card, creatureIndex) => {
      const id = CREATURE_IDS[creatureIndex],
        record = { ...emptyRecord, ...(save.records?.[id] || {}) };
      card
  ...
  if (screen.dataset.page === 'draft' && draftRun)
    screen
...
function renderCurrent() {
  if (battleSession && screen.classList.contains('battle-screen')) refreshBattle();
  else if (screen.dataset.page === 'title') renderTitle();
  else if (screen.dataset.page === 'selection') renderTeamSelect(selection.mode);
  ...
  else if (screen.dataset.page === 'results') renderResults(battleSession?.state.winner === 'player');
  else renderSettings();
}
...
function renderTitle() {
  ...
  const progress = save.ladderVictories,
    featured = CREATURE_IDS.filter((_, i) => i % 4 === Math.min(3, Math.floor(progress / 3))).slice(0, 6);
  const totalRanks = CREATURE_IDS.reduce((sum, id) => sum + masteryRank(save.mastery[id] || 0), 0);
  const primaryButton =
  ...
  screen.innerHTML = `<div class="shell"><header class="topbar"><div class="brand-small">✦ ${t('app.title')}</div><div class="icon-actions"><button class="icon-btn" data-action="toggle-mute" aria-label="${t('settings.mute')}" aria-pressed="${save.muted}">${save.muted ? '🔇' : '🔊'}</button><button class="icon-btn" data-action="settings" aria-label="${t('app.settings')}">⚙</button></div></header><div class="hero"><section class="hero-copy"><div class="eyebrow">24 CRÉATURES · 72 TECHNIQUES · 3 contre 3</div><h1>${i18n.lang === 'fr' ? 'Arène de' : 'Noam’s'} <span>${i18n.lang === 'fr' ? 'Noam' : 'Arena'}</span></h1><p>${t('app.tagline')}</p><div class="menu"><div class="menu-primary">${primaryButton}</div><div class="menu-group"><span class="menu-label">${t('title.battleModes')}</span><div class="menu-mode-grid">${actionButton(`${t('app.league')} <span>♜</span>`, 'league', 'league-btn')}${actionButton(`${t('app.gauntlet')} <span>↟</span>`, 'gauntlet', 'gauntlet-btn')}${actionButton(`${t('app.draft')} <span>◫</span>`, 'draft', 'draft-btn')}${actionButton(`${t('app.trials')} <span>♛</span>`, 'trials', 'trial-btn')}${actionButton(`${t('app.quick')} <span>⚡</span>`, 'quick')}</div></div><div class="menu-group menu-library"><span class="menu-label">${t('title.library')}</span><div class="menu-library-grid">${actionButton(`${t('app.bestiary')} <span>◈</span>`, 'bestiary')}${actionButton(`${t('app.academy')} <span>◎</span>`, 'academy')}</div></div></div></section><aside class="glass-panel progress-card"><div class="progress-orbit"></div><span class="eyebrow">${t('title.progress')}</span><h2>${t('title.emblems', { count: progress })}</h2>${save.winStreak ? `<div class="streak-banner"><i>🔥</i><span><b>${t('streak.current', { count: save.winStreak })}</b><small>${t('streak.best', { count: save.bestStreak })}</small></span></div>` : ''}<div class="career-strip"><span>★ ${t('mastery.total', { count: totalRanks })}</span><span>◆ ${t('feat.total', { count: save.feats.length })}</span><span>♛ ${save.trials.length}/${TRIALS.length}</span><span>↟ ${save.gauntletWins}</span><span>◫ ${save.draftWins}</span>${save.circuitWins ? `<span title="${t('circuit.title')}">♚ ${save.circuitWins}</span>` : ''}<span>☑ ${save.contractsCompleted}</span><span title="${t('grade.best')}">◇ ${save.bestGrade || '—'}</span><span title="${t('streak.best', { count: save.bestStreak })}">🔥 ${save.bestStreak}</span><span>⚔ ${save.wins}/${save.battlesPlayed}</span></div><button class="emblem-map" data-action="league" aria-label="${t('app.league')}"><div class="emblems epic-emblems">${TRAINERS.map((_, i) => emblemHtml(i, i < progress)).join('')}</div></button><div class="roster-fan">${featured.map((id) => `<img src="${sprite(id)}" alt="${creatureName(id)}">`).join('')}</div></aside></div></div>`;
  screen.querySelector('.hero-copy>.eyebrow').textContent = t('title.rosterLine');
  bindCommon();
  screen
    .querySelector('[data-action="play"]')
    ?.addEventListener('click', () => (save.tutorialComplete ? renderTeamSelect('ladder') : startTutorial()));
  screen
...
function openLeagueRival(index) {
  ...
  selection.difficulty = index >= 9 ? 'champion' : index >= 4 ? 'challenger' : save.difficulty;
...
function newSelection(mode) {
  const circuit = circuitMatch(save.circuitWins, LADDER_COUNT),
    index =
      mode === 'ladder'
        ? Math.min(save.ladderVictories, LADDER_COUNT - 1)
        : mode === 'circuit'
  ...
  return {
    mode,
    team: queryTeam('player', save.lastTeam),
    lead: 0,
...
function creatureCard(id, selected, lead, enemy = selection?.enemyTeam || []) {
  const c = CREATURES[id],
    a = affinity(id),
    passive = PASSIVES[c.passive],
    rank = masteryRank(save.mastery[id] || 0),
...
function renderTeamSelect(mode = 'ladder') {
  ...
  screen.className = 'screen';
  if (!selection || selection.mode !== mode) selection = newSelection(mode);
  const trainer = TRAINERS[selection.trainerIndex],
    circuit = mode === 'circuit' ? circuitMatch(save.circuitWins, LADDER_COUNT) : null,
    activeTrial = mode === 'trial' ? TRIALS.find((trial) => trial.id === selection.trialId) : null,
    ranked = ['ladder', 'circuit'].includes(mode);
  const matchup = teamMatchup(selection.team, selection.enemyTeam),
    squadProfile = teamProfile(selection.team),
    recommendedContract = {
      pressure: 'onslaught',
      control: 'tactician',
      sustain: 'guardian',
  ...
  const visibleIds =
    selection.filter === 'all'
      ? CREATURE_IDS
      : CREATURE_IDS.filter((id) => CREATURES[id].affinity === selection.filter);
  const affinityTabs = `<div class="affinity-tabs"><button class="affinity-tab ${selection.filter === 'all' ? 'active' : ''}" data-filter="all">24</button>${Object.entries(
    AFFINITIES
  )
    .filter(([id]) => id !== 'neutral')
    .map(
      ([id, a]) =>
        `<button class="affinity-tab ${selection.filter === id ? 'active' : ''}" data-filter="${id}" style="--tab-color:${a.color}">${a.icon} ${affinityName(id)}</button>`
    )
    .join('')}</div>`;
  const quickEnemyControls =
    mode === 'quick'
      ? `<div class="enemy-picker" aria-label="${t('select.enemy')}">${CREATURE_IDS.map((id) => `<button type="button" class="icon-btn ${selection.enemyTeam.includes(id) ? 'active' : ''}" data-enemy-pick="${id}" aria-label="${creatureName(id)}" aria-pressed="${selection.enemyTeam.includes(id)}"><img src="${sprite(id)}" alt=""></button>`).join('')}</div>${actionButton(t('app.random'), 'random-enemy', 'subtle-btn wide')}`
      : '';
  const arenaControl =
    mode === 'quick'
      ? `<div class="field"><label for="arena-select">${t('select.arena')}</label><select id="arena-select">${ARENAS.map((id) => `<option value="${id}" ${selection.arena === id ? 'selected' : ''}>${t(`arena.${id}`)}</option>`).join('')}</select></div><div class="field"><label for="quick-rule">${t('quickRule.title')}</label><select id="quick-rule">${QUICK_RULES.map((rule) => `<option value="${rule.id}" ${selection.quickRule === rule.id ? 'selected' : ''}>${rule.icon} ${t(`quickRule.${rule.id}`)}</option>`).join('')}</select></div><div class="battle-rule-preview"><b>${quickRule(selection.quickRule).icon} ${t(`quickRule.${selection.quickRule}`)}</b><span>${t(`quickRule.effect.${selection.quickRule}`)}</span></div>`
      : '';
  const chosenContract = CONTRACTS.find((contract) => contract.id === selection.contractId),
    contractControl =
      mode !== 'gauntlet'
        ? `<div class="field contract-field"><label for="contract-select">${t('contract.choose')}</label><select id="contract-select"><option value="random" ${selection.contractId === 'random' ? 'selected' : ''}>◇ ${t('contract.random')}</option>${CONTRACTS.map((contract) => `<option value="${contract.id}" ${selection.contractId === contract.id ? 'selected' : ''}>${contract.id === recommendedContract ? '★ ' : ''}${contract.icon} ${t(`contract.${contract.id}`)}</option>`).join('')}</select></div><div class="contract-preview ${chosenContract ? 'chosen' : 'random'} ${chosenContract?.id === recommendedContract ? 'recommended' : ''}">${chosenContract ? `<b>${chosenContract.id === recommendedContract ? '★ ' : ''}${chosenContract.icon} ${t(`contract.${chosenContract.id}`)}</b><span>${t(`contract.effect.${chosenContract.id}`, { target: chosenContract.target })}</span>` : `<b>◇ ${t('contract.random')}</b><span>${t('contract.randomHint')}</span><small>★ ${t('contract.suggested', { contract: t(`contract.${recommendedContract}`) })}</small>`}</div>`
        : '';
  const enemyRows =
    `<p class="meta-row">✦ ${t(`arena.${selection.arena}`)}</p>` +
    selection.enemyTeam
      .map(
        (id) =>
    ...
    (ranked
      ...
      : '');
  const scoutedLead = bestLeadIndex(selection.team, selection.enemyTeam),
    selectedRows = selection.team
      .map(
        (id, index) =>
          `<div class="selected-row ${index === scoutedLead ? 'recommended-lead' : ''}"><img src="${sprite(id)}" alt=""><span>${creatureName(id)}${index === scoutedLead ? `<small>◎ ${t('select.recommendedLead')}</small>` : ''}</span><button class="icon-btn" data-lead-index="${index}" aria-label="${t('select.chooseLead')}">${selection.lead === index ? '★' : '☆'}</button></div>`
      )
      .join('');
  const recommended = recommendedDoctrine(selection.team);
  const doctrines = ['balanced', 'assault', 'bastion', 'ambush']
    .map(
      (id) =>
        `<button type="button" class="doctrine-card ${selection.doctrine === id ? 'active' : ''} ${recommended === id ? 'recommended' : ''}" data-doctrine="${id}" aria-pressed="${selection.doctrine === id}">${recommended === id ? `<em>${t('profile.recommended')}</em>` : ''}<b>${t(`doctrine.icon.${id}`)} ${t(`doctrine.${id}`)}</b><small>${t(`doctrine.effect.${id}`)}</small></button>`
    )
    .join('');
  const presets = `<section class="squad-presets"><div class="squad-presets-head"><span><b class="eyebrow">${t('squad.title')}</b><small>${t('squad.hint')}</small></span>${actionButton(`⟳ ${t('squad.remix')}`, 'remix-team', 'subtle-btn remix-team-btn')}</div><div class="squad-preset-track">${SQUAD_PRESETS.map((preset) => `<button type="button" class="squad-preset ${preset.team.every((id, i) => selection.team[i] === id) ? 'active' : ''}" data-squad="${preset.id}"><i>${preset.icon}</i><span><b>${t(`squad.${preset.id}`)}</b><small>${t(`squad.effect.${preset.id}`)}</small></span><div>${preset.team.map((id) => `<img src="${sprite(id)}" alt="">`).join('')}</div></button>`).join('')}</div></section>`;
  const customSquads = `<section class="custom-squads"><div><span class="eyebrow">${t('loadout.title')}</span><small>${t('loadout.hint')}</small></div><div class="custom-squad-track">${Array.from(
    { length: 3 },
    (_, slot) => {
      const squad = save.customSquads?.[slot];
      return `<article class="custom-squad ${squad ? 'filled' : 'empty'}"><span><b>${t('loadout.slot', { slot: slot + 1 })}</b><small>${squad ? t(`doctrine.${squad.doctrine}`) : t('loadout.empty')}</small></span><div class="custom-squad-team">${squad ? squad.team.map((id, index) => `<i class="${index === squad.lead ? 'lead' : ''}"><img src="${sprite(id)}" alt="${creatureName(id)}"></i>`).join('') : '◇ ◇ ◇'}</div><div class="custom-squad-actions">${squad ? `<button type="button" data-custom-load="${slot}">${t('loadout.load')}</button><button type="button" data-custom-save="${slot}">${t('loadout.replace')}</button><button type="button" data-custom-clear="${slot}" aria-label="${t('loadout.clear')}">×</button>` : `<button type="button" data-custom-save="${slot}" ${selection.team.length === 3 ? '' : 'disabled'}>${t('loadout.save')}</button>`}</div></article>`;
    }
  ).join('')}</div></section>`;
  ...
  const selectionEyebrow =
      mode === 'circuit'
        ...
        : mode === 'ladder'
          ? `${t(trainer.nameKey)} · ${save.ladderVictories + 1}/${LADDER_COUNT}`
          : mode === 'gauntlet'
            ? t('gauntlet.subtitle')
            : activeTrial
  ...
  screen.innerHTML = `<div class="shell">${topbar()}<div class="page-head"><div><span class="eyebrow">${selectionEyebrow}</span><h1>${selectionTitle}</h1><p>${selectionSubtitle}</p></div><strong>${t('select.selected', { count: selection.team.length })}</strong></div>${circuitBanner}${customSquads}${presets}${affinityTabs}${kitShowcaseHtml(selection.team[selection.lead])}<div class="selection-layout"><div class="creature-grid">${visibleIds.map((id) => creatureCard(id, selection.team.includes(id), selection.team[selection.lead] === id)).join('')}</div><aside class="glass-panel select-aside">${ranked ? `<div class="trainer-badge">${emblemHtml(selection.trainerIndex, true)}</div>` : ''}<h2>${opponentTitle}</h2><div class="enemy-list">${enemyRows}</div>${quickEnemyControls}${!['gauntlet', 'circuit', 'trial'].includes(mode) ? `<div class="field"><label for="difficulty">${t('select.difficulty')}</label><select id="difficulty">${['apprentice', 'challenger', 'champion'].map((id) => `<option value="${id}" ${selection.difficulty === id ? 'selected' : ''}>${t(`difficulty.${id}`)}</option>`).join('')}</select></div>` : ''}${arenaControl}${contractControl}<div class="arena-rule"><b>${t('arena.ruleTitle')}</b><span>${t(`arena.rule.${selection.arena}`)}</span></div><h3>${t('doctrine.title')}</h3><div class="doctrine-picker">${doctrines}</div>${teamProfileHtml(selection.team)}<h3>${t('bond.title')}</h3>${bondsHtml(selection.team)}<h3>${t('combo.title')}</h3>${comboRoutesHtml(selection.team)}<h3>${t('select.matchup')}</h3><div class="matchup-line"><span class="match-pill good">↑ ${t('select.good')} ${matchup.good}</span><span class="match-pill risky">↓ ${t('select.risky')} ${matchup.risky}</span></div><div class="selected-list">${selectedRows}</div>${actionButton(readyLabel, 'start-battle', 'primary-btn wide', selection.team.length === 3 && selection.enemyTeam.length === 3 ? '' : 'disabled')}</aside></div></div>`;
  screen
    .querySelector('.selection-layout')
    ?.insertAdjacentHTML(
      'afterend',
      `<nav class="mobile-selection-dock" aria-label="${t('select.plan')}"><div>${selection.team.map((id, index) => `<i class="${selection.lead === index ? 'lead' : ''}"><img src="${sprite(id)}" alt="${creatureName(id)}"></i>`).join('')}</div><span><b>${t('select.selected', { count: selection.team.length })}</b><small>${t(`doctrine.${selection.doctrine}`)}</small></span><button type="button" data-action="open-plan">${t('select.plan')} ↑</button></nav>`
    );
  const difficultyField = screen.querySelector('#difficulty')?.closest('.field');
  difficultyField?.insertAdjacentHTML(
    'afterend',
    `<div class="difficulty-preview difficulty-${selection.difficulty}"><b>${t(`difficulty.${selection.difficulty}`)}</b><span>${t(`difficulty.effect.${selection.difficulty}`)}</span></div>`
  );
  if (mode === 'gauntlet')
    screen
  ...
  screen
    ...
    ?.addEventListener('click', () =>
      screen
        ...
        ?.scrollIntoView({ behavior: save.reducedMotion ? 'auto' : 'smooth', block: 'start' })
    );
  screen.querySelectorAll('[data-creature]').forEach((card) =>
    card.addEventListener('click', () => {
      ...
      if (index >= 0) {
        ...
        if (selection.lead >= selection.team.length) selection.lead = 0;
      } else if (selection.team.length < 3) selection.team.push(id);
  ...
  screen.querySelectorAll('[data-filter]').forEach((button) =>
    button.addEventListener('click', () => {
      selection.filter = button.dataset.filter;
      renderTeamSelect(mode);
    })
  );
  screen.querySelectorAll('[data-lead-index]').forEach((button) =>
    button.addEventListener('click', () => {
      selection.lead = Number(button.dataset.leadIndex);
      renderTeamSelect(mode);
    })
  );
  screen.querySelectorAll('[data-doctrine]').forEach((button) =>
    button.addEventListener('click', () => {
      selection.doctrine = button.dataset.doctrine;
      renderTeamSelect(mode);
    })
  );
  screen.querySelectorAll('[data-squad]').forEach((button) =>
    button.addEventListener('click', () => {
      ...
      selection.team = [...preset.team];
      selection.lead = preset.lead;
      selection.doctrine = preset.doctrine;
      selection.filter = 'all';
      renderTeamSelect(mode);
    })
  );
  screen.querySelector('[data-action="remix-team"]')?.addEventListener('click', () => {
    ...
    selection.team = remix.team;
    selection.lead = remix.lead;
    selection.doctrine = remix.doctrine;
    selection.filter = 'all';
    sound.ui();
    renderTeamSelect(mode);
  ...
  screen.querySelectorAll('[data-custom-load]').forEach((button) =>
    button.addEventListener('click', () => {
      const squad = save.customSquads?.[Number(button.dataset.customLoad)];
      if (!squad) return;
      selection.team = [...squad.team];
      selection.lead = squad.lead;
      selection.doctrine = squad.doctrine;
      selection.filter = 'all';
      sound.ui();
      renderTeamSelect(mode);
    })
  ...
  screen.querySelectorAll('[data-custom-save]').forEach((button) =>
    button.addEventListener('click', () => {
      ...
      save.customSquads = Array.from({ length: 3 }, (_, index) =>
        index === slot
          ? { team: [...selection.team], lead: selection.lead, doctrine: selection.doctrine }
          : save.customSquads?.[index] || null
      );
      persist();
  ...
  screen.querySelectorAll('[data-custom-clear]').forEach((button) =>
    button.addEventListener('click', () => {
      ...
      save.customSquads = Array.from({ length: 3 }, (_, index) =>
        index === slot ? null : save.customSquads?.[index] || null
      );
      persist();
      renderTeamSelect(mode);
    })
  ...
  screen.querySelector('[data-action="start-battle"]')?.addEventListener('click', () => {
    save.lastTeam = [...selection.team];
    if (['quick', 'ladder'].includes(mode)) save.difficulty = selection.difficulty;
    persist();
    if (mode === 'gauntlet') startGauntlet(selection.team, selection.lead, selection.doctrine);
    else
      startBattle({
        playerTeam: selection.team,
        enemyTeam: selection.enemyTeam,
        playerLead: selection.lead,
        enemyLead: 0,
        mode,
...
function startDraft() {
  ...
  draftRun = { ...createDraft(seed), team: [], round: 0, lead: 0, doctrine: 'balanced' };
  renderDraft();
}
...
function renderDraft() {
  ...
  screen.className = 'screen';
  const complete = draftRun.round >= draftRun.offers.length,
    offer = complete ? [] : draftRun.offers[draftRun.round],
    recommended = complete ? recommendedDoctrine(draftRun.team) : null,
    doctrines = ['balanced', 'assault', 'bastion', 'ambush']
      .map(
        (id) =>
          `<button type="button" class="doctrine-card ${draftRun.doctrine === id ? 'active' : ''} ${recommended === id ? 'recommended' : ''}" data-doctrine="${id}" aria-pressed="${draftRun.doctrine === id}">${recommended === id ? `<em>${t('profile.recommended')}</em>` : ''}<b>${t(`doctrine.icon.${id}`)} ${t(`doctrine.${id}`)}</b><small>${t(`doctrine.effect.${id}`)}</small></button>`
      )
      .join('');
  const scoutedDraftLead = complete ? bestLeadIndex(draftRun.team, draftRun.enemyTeam) : -1,
    lineup = Array.from({ length: 3 }, (_, index) => {
      const id = draftRun.team[index];
      return id
        ? `<button type="button" class="draft-slot filled ${draftRun.lead === index ? 'lead' : ''} ${scoutedDraftLead === index ? 'recommended' : ''}" data-draft-lead="${index}" aria-pressed="${draftRun.lead === index}"><span>${draftRun.lead === index ? '★' : index + 1}</span><img src="${sprite(id)}" alt=""><b>${creatureName(id)}${complete && scoutedDraftLead === index ? `<small>◎ ${t('select.recommendedLead')}</small>` : ''}</b></button>`
  ...
  const reveal = complete
    ? `<section class="draft-final"><div><span class="eyebrow">${t('draft.rival')}</span><h2>${t(`arena.${draftRun.arena}`)}</h2><div class="draft-rival-team">${draftRun.enemyTeam.map((id) => `<span><img src="${sprite(id)}" alt=""><b>${creatureName(id)}</b></span>`).join('')}</div><div class="arena-rule"><b>${t('arena.ruleTitle')}</b><span>${t(`arena.rule.${draftRun.arena}`)}</span></div></div><aside><h3>${t('doctrine.title')}</h3><div class="doctrine-picker">${doctrines}</div>${teamProfileHtml(draftRun.team)}<h3>${t('bond.title')}</h3>${bondsHtml(draftRun.team)}<h3>${t('combo.title')}</h3>${comboRoutesHtml(draftRun.team, true)}${actionButton(t('draft.enter'), 'draft-battle', 'primary-btn wide')}</aside></section>`
    : '';
  screen.innerHTML = `<div class="shell draft-page">${topbar()}<div class="draft-head"><span class="eyebrow">${t('draft.daily')} · #${draftRun.seed}</span><h1>${t('draft.title')}</h1><p>${complete ? t('draft.ready') : t('draft.pick', { round: draftRun.round + 1, total: draftRun.offers.length })}</p><div class="draft-progress">${draftRun.offers.map((_, index) => `<i class="${index < draftRun.round ? 'done' : index === draftRun.round && !complete ? 'active' : ''}"></i>`).join('')}</div></div><div class="draft-lineup">${lineup}</div>${complete ? reveal : `<div class="draft-offers">${offers}</div>`}</div>`;
  bindCommon();
  screen.querySelectorAll('[data-draft-pick]').forEach((button) =>
    button.addEventListener('click', () => {
      draftRun.team.push(button.dataset.draftPick);
      draftRun.round++;
      sound.ui();
      renderDraft();
    })
  ...
  screen.querySelectorAll('[data-draft-lead]').forEach((button) =>
    button.addEventListener('click', () => {
      draftRun.lead = Number(button.dataset.draftLead);
      sound.ui();
      renderDraft();
    })
  );
  screen.querySelectorAll('[data-doctrine]').forEach((button) =>
    button.addEventListener('click', () => {
      draftRun.doctrine = button.dataset.doctrine;
      renderDraft();
    })
  );
  screen.querySelector('[data-action="draft-battle"]')?.addEventListener('click', () => {
    save.lastTeam = [...draftRun.team];
    persist();
    startBattle({
      playerTeam: [...draftRun.team],
      enemyTeam: [...draftRun.enemyTeam],
      playerLead: draftRun.lead,
      enemyLead: 0,
      mode: 'draft',
      arena: draftRun.arena,
      difficulty: 'challenger',
      trainerIndex: draftRun.trainerIndex,
      doctrine: draftRun.doctrine,
      draftSeed: draftRun.seed,
    });
  });
}

function startGauntlet(team, lead, doctrine) {
  gauntletRun = { team: [...team], lead, doctrine, stage: 0, boons: [], condition: null };
  startGauntletStage();
}
function startGauntletStage() {
  const stage = GAUNTLET_STAGES[gauntletRun.stage],
    boonModifiers = gauntletRun.boons
      .map((id) => GAUNTLET_BOONS.find((x) => x.id === id)?.modifier)
      .filter(Boolean);
  startBattle({
    playerTeam: [...gauntletRun.team],
    enemyTeam: [...stage.enemyTeam],
    playerLead: gauntletRun.lead,
    enemyLead: 0,
    mode: 'gauntlet',
    arena: stage.arena,
    ...
    doctrine: gauntletRun.doctrine,
    playerCondition: gauntletRun.condition,
  });
}

function advanceGauntlet() {
  gauntletRun.condition = Object.fromEntries(
    battleSession.state.sides.player.team.map((creature) => [
      creature.id,
      creature.hp > 0 ? Math.min(1, creature.hp / creature.maxHp + 0.24) : 0.4,
    ])
  );
  gauntletRun.stage++;
  if (gauntletRun.stage >= GAUNTLET_STAGES.length) {
    save.gauntletWins = Math.min(999, save.gauntletWins + 1);
    persist();
    renderResults(true);
    return;
  }
...
function renderGauntletBoons() {
  ...
  screen.className = 'screen';
  const available = GAUNTLET_BOONS.filter((boon) => !gauntletRun.boons.includes(boon.id)),
    next = GAUNTLET_STAGES[gauntletRun.stage],
    mastery =
      pendingRewards?.mastery
        .map(
          (reward) =>
        ...
        .join('') || '',
    feats = pendingRewards?.newFeats.length
      ? `<div class="feat-rewards"><strong>${t('feat.unlocked')}</strong>${pendingRewards.newFeats.map((id) => `<div><b>${FEATS[id].icon} ${t(`feat.${id}`)}</b><span>${t(`feat.effect.${id}`)}</span></div>`).join('')}</div>`
      : '',
    contractReward = pendingRewards?.contractComplete
      ? `<div class="contract-reward">☑ <b>${t('contract.complete')}</b><span>${t('contract.masteryBonus')}</span></div>`
      : '';
  screen.innerHTML = `<div class="shell">${topbar()}<div class="gauntlet-reward"><section class="glass-panel"><span class="eyebrow">${t('gauntlet.roundClear', { round: gauntletRun.stage, total: GAUNTLET_STAGES.length })}</span><h1>${t('gauntlet.chooseBoon')}</h1><p>${t('gauntlet.chooseBoonHint')}</p>${performanceHtml(pendingRewards?.grade, true)}${contractReward}<div class="mastery-rewards">${mastery}</div>${feats}<div class="boon-grid">${available.map((boon) => `<button type="button" class="boon-card" data-boon="${boon.id}"><i>${boon.icon}</i><span><b>${t(`boon.${boon.id}`)}</b><small>${t(`boon.effect.${boon.id}`)}</small></span></button>`).join('')}</div><div class="next-gauntlet"><span>${t('gauntlet.next')}</span><b>${t(next.nameKey)} · ${t(`arena.${next.arena}`)}</b><div>${next.enemyTeam.map((id) => `<img src="${sprite(id)}" alt="${creatureName(id)}">`).join('')}</div></div></section></div></div>`;
  const scoutedLead = bestLeadIndex(gauntletRun.team, next.enemyTeam),
    camp = `<div class="gauntlet-condition"><div><span class="eyebrow">${t('gauntlet.camp')}</span><small>${t('gauntlet.campHint')}</small></div>${gauntletRun.team
      .map((id, index) => {
        const ratio = gauntletRun.condition?.[id] || 1;
        return `<button type="button" class="${gauntletRun.lead === index ? 'lead' : ''} ${scoutedLead === index ? 'recommended' : ''}" data-gauntlet-lead="${index}" aria-pressed="${gauntletRun.lead === index}"><img src="${sprite(id)}" alt=""><b>${creatureName(id)}${scoutedLead === index ? `<small>◎ ${t('select.recommendedLead')}</small>` : ''}</b><i><u style="width:${ratio * 100}%"></u></i><em>${Math.round(ratio * 100)}%</em></button>`;
      })
      .join('')}</div>`;
  ...
  screen.querySelectorAll('[data-gauntlet-lead]').forEach((button) =>
    button.addEventListener('click', () => {
      gauntletRun.lead = Number(button.dataset.gauntletLead);
      sound.ui();
      renderGauntletBoons();
    })
  );
  screen.querySelectorAll('[data-boon]').forEach((button) =>
    button.addEventListener('click', () => {
      gauntletRun.boons.push(button.dataset.boon);
      startGauntletStage();
    })
  );
}
...
function renderTrials() {
  ...
  screen.className = 'screen';
  const squad = save.lastTeam
    .map((id) => `<span><img src="${sprite(id)}" alt=""><b>${creatureName(id)}</b></span>`)
    .join('');
  screen.innerHTML = `<div class="shell">${topbar()}<div class="page-head"><div><span class="eyebrow">${save.trials.length}/${TRIALS.length} ${t('trial.cleared')}</span><h1>${t('trial.title')}</h1><p>${t('trial.subtitle')}</p></div><div class="trial-squad"><small>${t('trial.squad')}</small>${squad}</div></div><div class="trial-grid">${TRIALS.map((trial, index) => `<article class="trial-card ${save.trials.includes(trial.id) ? 'cleared' : ''}" style="--trial-a:${trial.colors[0]};--trial-b:${trial.colors[1]}"><div class="trial-sigil">${trial.icon}</div><span class="eyebrow">${t(`difficulty.${trial.difficulty}`)} · ${t(`arena.${trial.arena}`)}</span><h2>${t(trial.nameKey)}</h2><p>${t(trial.descKey)}</p><div class="trial-enemies">${trial.enemyTeam.map((id) => `<img src="${sprite(id)}" alt="${creatureName(id)}" title="${creatureName(id)}">`).join('')}</div>${save.trials.includes(trial.id) ? `<strong class="trial-clear">✓ ${t('trial.complete')}</strong>` : ''}${actionButton(t('trial.challenge'), `trial-${index}`, 'primary-btn wide')}</article>`).join('')}</div></div>`;
  bindCommon();
...
function startBattle(config) {
  ...
  const state = createBattle({
    ...
    enemyAce: ['ladder', 'gauntlet', 'circuit'].includes(config.mode)
      ...
      : null,
    masteryRanks: Object.fromEntries(config.playerTeam.map((id) => [id, masteryRank(save.mastery[id] || 0)])),
  });
...
function renderBattle() {
  ...
  const trial = battleSession.mode === 'trial' ? TRIALS.find((x) => x.id === battleSession.trialId) : null;
  const gauntlet = battleSession.mode === 'gauntlet' ? GAUNTLET_STAGES[battleSession.gauntletStage] : null;
  const circuit = battleSession.mode === 'circuit' ? circuitMatch(save.circuitWins, LADDER_COUNT) : null;
  const resonance = ARENA_RESONANCE[battleSession.arena],
    resonanceMeta = AFFINITIES[resonance];
  screen.innerHTML = `<canvas id="arena" class="arena-canvas" aria-hidden="true"></canvas><div class="battle-vignette"></div><div class="battle-top"><span class="turn-chip" id="turn-chip"></span><div class="arena-nameplate"><b>${trial ? `${trial.icon} ${t(trial.nameKey)}` : gauntlet ? `↟ ${t(gauntlet.nameKey)} · ${battleSession.gauntletStage + 1}/${GAUNTLET_STAGES.length}` : circuit ? `${circuit.condition.icon} ${t('circuit.round', { round: circuit.round })}` : t(`arena.${battleSession.arena}`)}</b><small>${trial ? t(trial.descKey) : gauntlet ? t('gauntlet.battleRule', { boons: gauntletRun?.boons.length || 0 }) : circuit ? t(`circuit.effect.${circuit.condition.id}`) : t(`arena.rule.${battleSession.arena}`)}</small>${resonanceMeta ? `<span class="arena-resonance" style="--resonance-color:${resonanceMeta.color}">${resonanceMeta.icon} ${t('arena.resonanceShort', { affinity: affinityName(resonance) })}</span>` : ''}${battleSession.contractId ? `<div class="contract-chip" id="contract-chip"></div>` : ''}</div><div class="battle-tools"><button class="icon-btn" data-action="battle-help" aria-label="${t('battle.codex')}">?</button><button class="icon-btn" data-action="battle-speed" aria-pressed="${save.battleSpeed === 2}">×${save.battleSpeed}</button><button class="icon-btn" data-action="toggle-mute" aria-label="${t('settings.mute')}">${save.muted ? '🔇' : '🔊'}</button><button class="icon-btn" data-action="battle-exit" aria-label="${t('app.back')}">✕</button></div></div><div class="battlefield"><div class="fighter enemy" id="fighter-enemy"><div class="status-orbits"></div><img alt=""></div><div class="fighter player" id="fighter-player"><div class="status-orbits"></div><img alt=""></div></div><div class="hud-card enemy-hud" id="hud-enemy"></div><div class="hud-card player-hud" id="hud-player"></div><div class="action-line" id="action-line"></div><div class="battle-controls"><div class="move-grid" id="moves"></div><button class="switch-btn" data-action="open-switch"><span>↺</span>${t('battle.switch')}</button></div><div id="replacement-root"></div><div id="tutorial-root"></div>`;
  if (battleSession.quickRuleId && battleSession.quickRuleId !== 'standard') {
    const rule = quickRule(battleSession.quickRuleId);
    screen
  ...
  screen
    ...
    ?.insertAdjacentHTML(
      'afterbegin',
      `<button class="icon-btn trainer-command-btn" data-action="trainer-command" aria-label="${t('battle.command')}"><span>⚑</span><small>${t(`command.${battleSession.state.doctrine}`)}</small></button>`
    );
  ...
  try {
    ...
    arenaScene = new ArenaScene(screen.querySelector('#arena'), battleSession.arena, {
  ...
  screen.querySelector('[data-action="battle-speed"]').addEventListener('click', () => {
    save.battleSpeed = save.battleSpeed === 2 ? 1 : 2;
    persist();
    refreshBattle();
  });
...
function openBattleCodex() {
  ...
  const state = battleSession.state,
    ...
    boons = gauntletRun?.boons || [],
    activeRule = battleSession.quickRuleId ? quickRule(battleSession.quickRuleId) : null,
    circuit = battleSession.mode === 'circuit' ? circuitMatch(save.circuitWins, LADDER_COUNT) : null,
    trainerAce = state.enemyAce,
    contract = CONTRACTS.find((item) => item.id === battleSession.contractId);
...
function openBattleLog() {
  ...
  if (!root) return;
  const entries = [...(battleSession.timeline || [])].reverse(),
    icons = { player: '◆', enemy: '◇' };
...
async function battleEntrance() {
  if (testAnimationScale === 0 || !battleSession) return;
  locked = true;
  refreshBattle();
  const stage = screen.querySelector('#fx-stage'),
    ...
    trainer = ['ladder', 'circuit'].includes(battleSession.mode)
      ? TRAINERS[battleSession.trainerIndex]
      : null,
    trial = battleSession.mode === 'trial' ? TRIALS.find((x) => x.id === battleSession.trialId) : null;
  const gauntletTrainer = battleSession.mode === 'gauntlet' ? TRAINERS[battleSession.trainerIndex] : null,
  ...
  setTimeout(() => sound.call(enemy.id), 220 / save.battleSpeed);
  await wait((save.reducedMotion ? 300 : 1380) / save.battleSpeed);
  clearBattleFx();
  locked = false;
  if (battleSession?.state.phase === 'choice') {
    battleSession.lastLine = t('battle.yourTurn');
    refreshBattle();
  }
}
...
function enemyIntentHtml() {
  const state = battleSession.state;
  if (locked || state.phase !== 'choice' || battleSession.mode === 'tutorial') return '';
  const level = battleSession.difficulty || 'apprentice';
  if (level === 'champion')
    return `<div class="intent-read hidden"><span>◉ ${t('battle.intent')}</span><b>${t('battle.intentHidden')}</b></div>`;
  const action = chooseAiAction(state, 'enemy', level, battleSession.style);
  if (!action) return '';
  let icon = '↺',
...
function enemyPlan() {
  if (
    locked ||
    battleSession.state.phase !== 'choice' ||
    battleSession.mode === 'tutorial' ||
    battleSession.difficulty === 'champion'
  )
    return null;
  return chooseAiAction(
    battleSession.state,
    'enemy',
    battleSession.difficulty || 'apprentice',
    battleSession.style
  );
}
...
function hudHtml(side) {
  const state = battleSession.state,
    ...
    passive = PASSIVES[c.passive],
    rank = side === 'player' ? masteryRank(save.mastery[c.id] || 0) : 0;
...
function moveButton(moveId, index) {
  ...
  const plan = battleSession.difficulty === 'apprentice' ? enemyPlan() : null,
  ...
  const tutorialAllowed =
    battleSession.mode !== 'tutorial' ||
    battleSession.tutorialStep >= 3 ||
    (battleSession.tutorialStep === 0 && move.kind === 'damage') ||
    (battleSession.tutorialStep === 1 && moveId === 'oracle_veil');
...
function refreshBattle() {
  if (!battleSession || !screen.classList.contains('battle-screen')) return;
  const state = battleSession.state,
  ...
  arenaScene?.setBattleState({ tension, imminent: until === 1 });
  ...
  screen.querySelector('#action-line').textContent = battleSession.lastLine;
  for (const side of ['player', 'enemy']) {
    const owner = state.sides[side],
      c = activeOf(state, side),
  ...
  const forecastPlan = battleSession.difficulty === 'apprentice' && !locked ? enemyPlan() : null;
  if (forecastPlan)
    screen
  ...
  switchButton.disabled =
    locked ||
    (battleSession.mode === 'tutorial' && battleSession.tutorialStep < 2) ||
  ...
  speedButton.textContent = `×${save.battleSpeed}`;
  speedButton.setAttribute('aria-pressed', String(save.battleSpeed === 2));
  const mute = screen.querySelector('[data-action="toggle-mute"]');
  mute.textContent = save.muted ? '🔇' : '🔊';
  ...
  if (commandButton) {
    ...
    commandButton.disabled = locked || used || state.phase !== 'choice';
  ...
  if (contractEl) {
    const contract = CONTRACTS.find((x) => x.id === battleSession.contractId),
...
function renderTutorialTip() {
  ...
  if (!root || battleSession.mode !== 'tutorial') {
    if (root) root.innerHTML = '';
    return;
  }
  const step = Math.min(3, battleSession.tutorialStep);
...
function openSwitch() {
  ...
  const state = battleSession.state,
    ...
    plan =
      !state.sides.player.pendingReplacement && battleSession.difficulty === 'apprentice'
        ? enemyPlan()
        : null;
...
async function handleTrainerCommand() {
  if (locked || battleSession.state.phase !== 'choice' || battleSession.state.sides.player.commandUsed)
    return;
  await sound.unlock();
  locked = true;
  refreshBattle();
  const result = applyTrainerCommand(battleSession.state, 'player');
...
async function handlePlayerAction(action) {
  ...
  locked = true;
  refreshBattle();
  const tutorialStep = battleSession.tutorialStep;
  const enemyAction =
    battleSession.mode === 'tutorial'
      ? tutorialEnemyAction(tutorialStep)
      : chooseAiAction(battleSession.state, 'enemy', battleSession.difficulty, battleSession.style);
  if (battleSession.mode === 'tutorial') {
    if (tutorialStep === 0 && action.type === 'move') battleSession.tutorialStep = 1;
    else if (tutorialStep === 1 && action.moveId === 'oracle_veil') battleSession.tutorialStep = 2;
    else if (tutorialStep === 2 && action.type === 'switch') battleSession.tutorialStep = 3;
  }
  const result = resolveTurn(battleSession.state, action, enemyAction);
  battleSession.state = result.state;
  await playEvents(result.events);
  locked = false;
  if (battleSession.state.phase === 'ended') {
    finishBattle();
    return;
  }
...
function tutorialEnemyAction(step) {
  const legal = getLegalActions(battleSession.state, 'enemy');
  if (step === 1) {
    const switchToCalderoc = legal.find(
      (action) =>
        action.type === 'switch' && battleSession.state.sides.enemy.team[action.index].id === 'calderoc'
    );
...
function impactMoveFx(event) {
  ...
  if (event.hp <= 0) {
    ...
    battleSession.lastLine = t('battle.finisher', { move: t(`move.${fx.moveId}`) });
    screen.querySelector('#action-line').textContent = battleSession.lastLine;
  }
  const color = AFFINITIES[fx.affinity]?.color || '#ffffff';
  arenaScene?.flash(fx.strong ? 'power' : 'hit', color, event.side);
  arenaScene?.punch(event.side, fx.strong ? 1.55 : (event.hits || 1) > 1 ? 0.8 : 1);
  screen.classList.add('hit-stop');
  setTimeout(() => screen.classList.remove('hit-stop'), save.reducedMotion ? 20 : 72 / save.battleSpeed);
}
...
function tacticalFx(event) {
  ...
  const moveClass = currentFxMove?.moveId ? `move-${currentFxMove.moveId}` : '',
  ...
  arenaScene?.burst(color, side, event.type === 'heal' ? 1.2 : 0.8);
}
...
function detonationFx(event) {
  ...
  stage.append(call);
  arenaScene?.flash('power', meta.color, event.side);
  arenaScene?.punch(event.side, 1.15);
...
function assistFx(event) {
  ...
  arenaScene?.burst(a.color, event.side, 1);
...
function perfectRelayFx(event) {
  ...
  arenaScene?.flash('hit', a.color, event.side);
  arenaScene?.burst(a.color, event.side, 1.2);
  sound.guard();
  sound.ui();
}
...
function relayRushFx(event) {
  const stage = screen.querySelector('#fx-stage'),
    creature = activeOf(battleSession.state, event.side);
  ...
  arenaScene?.flash('power', a.color, event.side);
  arenaScene?.burst(a.color, event.side, 1.5);
  arenaScene?.punch(event.side, 1.15);
...
function flowCrescendoFx(event) {
  ...
  arenaScene?.flash('power', a.color, event.side);
  arenaScene?.burst(a.color, event.side, 1.25);
  sound.clash();
  setTimeout(
    () => {
      call.remove();
    ...
    save.reducedMotion ? 220 : Math.max(700, 620 / save.battleSpeed)
  );
}
...
function trainerCommandFx(event) {
  ...
  arenaScene?.flash('power', a.color, 'player');
  arenaScene?.burst(a.color, 'player', 1.25);
  sound.clash();
}
...
function signatureReadyFx(event) {
  ...
  stage.append(call);
  arenaScene?.flash('power', a.color, event.side);
  arenaScene?.burst(a.color, event.side, 1.15);
  sound.call(creature.id);
  sound.ui();
  setTimeout(() => call.remove(), (save.reducedMotion ? 180 : 900) / save.battleSpeed);
}
...
function finalDuelFx(event) {
  ...
  arenaScene?.flash('power', '#fff09a', 'enemy');
  arenaScene?.burst(AFFINITIES[player.affinity].color, 'player', 1.6);
  arenaScene?.burst(AFFINITIES[enemy.affinity].color, 'enemy', 1.6);
  sound.clash();
}
...
function resonanceFx(event) {
  ...
  arenaScene?.burst(a.color, event.side, 1.3);
...
function aceFx(event) {
  ...
  arenaScene?.flash('power', a.color, 'enemy');
  arenaScene?.burst(a.color, 'enemy', 1.7);
  arenaScene?.punch('enemy', 1.3);
  sound.clash();
}
...
function statusTickFx(event) {
  ...
  arenaScene?.burst(meta?.color || '#fff', side, 0.8);
}
...
function arenaPulseFx(event) {
  ...
  arenaScene?.flash('power', color, 'enemy');
  arenaScene?.burst(color, 'player', 1.4);
  arenaScene?.burst(color, 'enemy', 1.4);
  sound.guard();
}
...
async function signatureClashIntro(events) {
  ...
  battleSession.lastLine = t('battle.signatureClash');
  screen.querySelector('#action-line').textContent = battleSession.lastLine;
  sound.clash();
  await wait((save.reducedMotion ? 260 : 1050) / save.battleSpeed);
  clearBattleFx();
}
...
function eventPresentationDelay(event) {
  ...
  if (save.reducedMotion) return 190 / save.battleSpeed;
  if (event.type === 'move-start')
    return (
      (MOVES[event.moveId]?.signature ? 780 : (MOVES[event.moveId]?.power || 0) >= 46 ? 580 : 430) /
      save.battleSpeed
    );
  if (event.type === 'trainer-command') return 760 / save.battleSpeed;
  if (event.type === 'damage')
    return (
      (event.hp <= 0 ? 900 : event.affinity !== 1 ? 700 : currentFxMove?.strong ? 620 : 430) /
      save.battleSpeed
    );
  if (event.type === 'arena-pulse') return 760 / save.battleSpeed;
  if (event.type === 'rally') return 620 / save.battleSpeed;
  if (event.type === 'surge')
    return (
      (event.source === 'switch' && event.amount >= 24 ? 650 : event.ready ? 760 : 100) / save.battleSpeed
    );
  if (event.type === 'assist') return 480 / save.battleSpeed;
  if (event.type === 'perfect-relay') return 620 / save.battleSpeed;
  if (event.type === 'final-duel') return 1050 / save.battleSpeed;
  if (event.type === 'resonance') return 560 / save.battleSpeed;
  if (event.type === 'flow') return (event.count === 3 ? 620 : 220) / save.battleSpeed;
  if (event.type === 'ace') return 1050 / save.battleSpeed;
  if (event.type === 'ko') return 700 / save.battleSpeed;
  if (['heal', 'status', 'barrier', 'barrier-hit', 'miss', 'recoil', 'status-tick'].includes(event.type))
    return 460 / save.battleSpeed;
  return 300 / save.battleSpeed;
}
...
async function playEvents(events) {
  ...
  for (let eventIndex = 0; eventIndex < events.length; eventIndex++) {
    ...
    if (event.type === 'trainer-command') {
      battleSession.lastLine = t('battle.commandLine', { command: t(`command.${event.command}`) });
      trainerCommandFx(event);
    }
    if (event.type === 'move-start') {
      battleSession.lastLine = t('battle.action.move', {
    ...
    if (event.type === 'assist') {
      battleSession.lastLine = t('battle.assist', {
    ...
    if (event.type === 'perfect-relay') {
      battleSession.lastLine = t('battle.perfectRelay', { actor: creatureName(event.creatureId) });
      perfectRelayFx(event);
    }
    if (event.type === 'final-duel') {
      battleSession.lastLine = t('battle.finalDuelLine');
      finalDuelFx(event);
    }
    if (event.type === 'flow') {
      const refreshed = Boolean(event.refreshed?.length),
        flowLine =
          event.count === 3
            ...
            : 'battle.flowLine';
      battleSession.lastLine = t(flowLine, { count: event.count, surge: event.surge });
      ...
      const affinityNote =
        event.affinity === 1.5
          ...
          : event.affinity === 0.75
            ? `↓ ${t('battle.resisted')} · `
            : '';
      battleSession.lastLine = `${affinityNote}${event.combo?.length ? `${t('battle.combo')} · ` : ''}${t('battle.action.damage', { target: creatureName(event.creatureId), amount: event.amount })}${event.hits > 1 ? ` · ${t('battle.hit', { hit: event.hit, hits: event.hits })}` : ''}`;
      fighter?.classList.add('hit');
      impactMoveFx(event);
      sound.impact(MOVES[currentFxMove?.moveId], event);
    }
    if (event.type === 'heal') {
      battleSession.lastLine = t('battle.action.heal', {
        actor: creatureName(event.creatureId),
        amount: event.amount,
      });
      tacticalFx(event);
    ...
    if (event.type === 'status') {
      battleSession.lastLine = event.detonated
      ...
      if (event.detonated) {
        screen.classList.add('combo-hit');
        detonationFx(event);
        arenaScene?.burst(STATUS_DEFINITIONS[event.status]?.color || '#fff', event.side, 1.35);
      } else {
        tacticalFx(event);
        sound.guard();
      }
    ...
    if (event.type === 'barrier') {
      battleSession.lastLine = t('battle.action.barrier', {
        actor: creatureName(event.creatureId),
        amount: event.amount,
      });
    ...
    if (event.type === 'barrier-hit') {
      battleSession.lastLine = t('battle.action.absorb', { amount: event.amount });
      fighter?.classList.add('barrier-hit');
      arenaScene?.flash('hit', '#73eaff', event.side);
      sound.guard();
    }
    if (event.type === 'miss') {
      battleSession.lastLine = t('battle.action.miss', { actor: creatureName(event.creatureId) });
      fighter?.classList.add('dodging');
      sound.ui();
    }
    if (event.type === 'recoil') {
      battleSession.lastLine = t('battle.action.recoil', {
        actor: creatureName(event.creatureId),
        amount: event.amount,
      });
      fighter?.classList.add('hit');
    ...
    if (event.type === 'status-tick') {
      battleSession.lastLine = t('battle.action.tick', {
        actor: creatureName(event.creatureId),
        amount: event.amount,
        status: t(`status.${event.status}`),
      });
    ...
    if (event.type === 'surge' && event.source === 'switch' && event.amount >= 24) {
      const incoming = activeOf(battleSession.state, event.side);
      battleSession.lastLine = t('battle.relayRushLine', { actor: creatureName(incoming.id) });
      relayRushFx(event);
      screen.querySelector(`#hud-${event.side}`)?.classList.add('surge-flash');
    }
    if (event.type === 'surge' && event.source === 'mastery') {
      battleSession.lastLine = t('battle.masterySpark');
      screen.querySelector(`#hud-${event.side}`)?.classList.add('surge-flash');
      sound.ui();
    }
    if (event.type === 'surge' && event.ready) {
      battleSession.lastLine = t('battle.surgeReady');
      screen.querySelector(`#hud-${event.side}`)?.classList.add('surge-flash');
      signatureReadyFx(event);
    }
    if (event.type === 'arena-pulse') {
      battleSession.lastLine = t('battle.arenaPulse', { arena: t(`arena.${event.arena}`) });
      arenaPulseFx(event);
    }
    if (event.type === 'resonance') {
      battleSession.lastLine = t('battle.resonanceLine', {
        actor: creatureName(event.creatureId),
    ...
    if (event.type === 'ace') {
      battleSession.lastLine = t('battle.ace', {
        actor: creatureName(event.creatureId),
        ace: t(`ace.${event.ace}`),
      });
      aceFx(event);
    ...
    if (event.type === 'rally') {
      battleSession.lastLine = t('battle.rally', { actor: creatureName(event.creatureId) });
      tacticalFx({ ...event, status: 'focused' });
      screen.classList.add('rally-beat');
      sound.victory();
    }
    if (event.type === 'passive') {
      battleSession.lastLine = t('battle.passive', {
        actor: creatureName(event.creatureId),
    ...
    if (event.type === 'switch' || event.type === 'replace') {
      battleSession.lastLine = t('battle.action.switch', { actor: creatureName(event.creatureId) });
      sound.ui();
    }
    if (event.type === 'ko') {
      battleSession.lastLine = t('battle.ko', { name: creatureName(event.creatureId) });
      fighter?.classList.add('ko');
      screen.classList.add('ko-shock');
      sound.ko();
    }
    if (event.type === 'battle-end' && event.reason === 'turn-cap') battleSession.lastLine = t('battle.cap');
    if (LOG_EVENT_TYPES.has(event.type)) {
      battleSession.timeline.push({
        type: event.type,
        side: event.side,
        turn: event.turn || battleSession.state.turn,
        text: battleSession.lastLine,
      });
      if (battleSession.timeline.length > 40) battleSession.timeline.shift();
    }
    refreshBattle();
...
async function resolvePendingReplacements() {
  ...
  if (state.sides.enemy.pendingReplacement) {
    const action = chooseAiAction(state, 'enemy', battleSession.difficulty, battleSession.style);
  ...
  if (state.sides.player.pendingReplacement) {
    battleSession.lastLine = t('battle.chooseReplacement');
    refreshBattle();
    openSwitch();
  }
}
...
async function handleReplacement(index) {
  locked = true;
  const result = applyReplacement(battleSession.state, 'player', { type: 'replace', index });
  battleSession.state = result.state;
  await playEvents(result.events);
  locked = false;
  await resolvePendingReplacements();
...
function awardBattleProgress(
  ...
  for (const creature of state.sides.player.team) {
    const before = save.mastery[creature.id] || 0,
      gain =
        1 +
        (win ? 2 : 0) +
    ...
    save.mastery[creature.id] = after;
    mastery.push({
      id: creature.id,
      gain,
  ...
  for (const creature of state.sides.player.team) {
    const id = creature.id,
      previous = save.records[id] || { battles: 0, wins: 0, damage: 0, kos: 0, signatures: 0, assists: 0 },
  ...
  for (const id of candidates)
    if (!save.feats.includes(id)) {
      save.feats.push(id);
      newFeats.push(id);
    }
  const previousStreak = save.winStreak || 0;
  save.battlesPlayed = Math.min(9999, save.battlesPlayed + 1);
  if (win) {
    save.wins = Math.min(9999, save.wins + 1);
    save.winStreak = Math.min(9999, previousStreak + 1);
    save.bestStreak = Math.max(save.bestStreak || 0, save.winStreak);
  } else save.winStreak = 0;
  return {
    mastery,
    newFeats,
    contractComplete,
    grade,
    streak: { current: save.winStreak, best: save.bestStreak, broken: win ? 0 : previousStreak },
  };
}
...
function finishBattle() {
  const state = battleSession.state;
  if (battleSession.mode === 'tutorial') {
    setTimeout(completeTutorial, 500 / save.battleSpeed);
    return;
  }
  const win = state.winner === 'player',
    contract = CONTRACTS.find((x) => x.id === battleSession.contractId),
    contractComplete = Boolean(contract && contractProgress(contract.id, state.history) >= contract.target),
    grade = gradeBattle(state, win, contractComplete);
  if (win && battleSession.mode === 'ladder') {
    const index = battleSession.trainerIndex;
    if (index === save.ladderVictories && save.ladderVictories < LADDER_COUNT) {
      save.ladderVictories++;
      const emblem = TRAINERS[index].id;
      if (!save.emblems.includes(emblem)) save.emblems.push(emblem);
      for (const unlocked of TRAINERS.slice(0, save.ladderVictories).map((x) => x.arena))
        if (!save.cosmetics.includes(unlocked)) save.cosmetics.push(unlocked);
    }
  }
  if (win && battleSession.mode === 'trial' && !save.trials.includes(battleSession.trialId))
    save.trials.push(battleSession.trialId);
  if (win && battleSession.mode === 'draft') save.draftWins = Math.min(9999, save.draftWins + 1);
  if (win && battleSession.mode === 'circuit') save.circuitWins = Math.min(9999, save.circuitWins + 1);
  if (contractComplete) save.contractsCompleted = Math.min(9999, save.contractsCompleted + 1);
  if (
    !save.bestGrade ||
    PERFORMANCE_GRADES.indexOf(grade.letter) > PERFORMANCE_GRADES.indexOf(save.bestGrade)
  )
    save.bestGrade = grade.letter;
  pendingRewards = awardBattleProgress(state, win, contractComplete, grade);
  persist();
  if (win && battleSession.mode === 'gauntlet') {
    setTimeout(advanceGauntlet, 450 / save.battleSpeed);
    return;
  }
  setTimeout(() => renderResults(win), 450 / save.battleSpeed);
}
...
function renderResults(win) {
  ...
  const progress =
    battleSession.mode === 'ladder'
      ? `<div><span class="eyebrow">${t('title.progress')}</span><div class="emblems epic-emblems">${TRAINERS.map((_, i) => emblemHtml(i, i < save.ladderVictories)).join('')}</div></div>`
      : battleSession.mode === 'trial' && win
        ? `<div class="trial-victory">♛ ${t('trial.reward', { count: save.trials.length, total: TRIALS.length })}</div>`
        : battleSession.mode === 'gauntlet' && win
          ? `<div class="gauntlet-victory">↟ ${t('gauntlet.conquered', { count: save.gauntletWins })}</div>`
          : battleSession.mode === 'draft' && win
            ? `<div class="draft-victory">◫ ${t('draft.won', { count: save.draftWins })}</div>`
            : battleSession.mode === 'circuit' && win
              ? `<div class="circuit-victory">♚ ${t('circuit.won', { count: save.circuitWins })}</div>`
              : '';
  const mastery =
    pendingRewards?.mastery
      .map(
      ...
      .join('') || '';
  const feats = pendingRewards?.newFeats.length
    ? `<div class="feat-rewards"><strong>${t('feat.unlocked')}</strong>${pendingRewards.newFeats.map((id) => `<div><b>${FEATS[id].icon} ${t(`feat.${id}`)}</b><span>${t(`feat.effect.${id}`)}</span></div>`).join('')}</div>`
    : '';
  const contractReward = pendingRewards?.contractComplete
    ? `<div class="contract-reward">☑ <b>${t('contract.complete')}</b><span>${t('contract.masteryBonus')}</span></div>`
    : '';
  const streak = pendingRewards?.streak,
    streakReward =
  ...
  const celebration = win
    ...
    : '';
  screen.innerHTML = `<div class="shell result-page">${topbar()}${celebration}<div class="result-shell"><section class="glass-panel result-card ${win ? 'won' : 'lost'}"><div class="result-icon">${win ? '✦' : '◇'}</div><h1>${win ? t('result.victory') : t('result.defeat')}</h1><p>${win ? t('result.victoryText') : t('result.defeatText')}</p>${performanceHtml(pendingRewards?.grade)}${streakReward}<div class="result-team">${team.map((c) => `<img class="${c.hp <= 0 ? 'fallen' : ''}" src="${sprite(c.id)}" alt="${creatureName(c.id)}">`).join('')}</div><div class="battle-recap"><div class="recap-mvp"><img src="${sprite(recap.mvp.id)}" alt=""><span><small>${t('result.mvp')}</small><b>${creatureName(recap.mvp.id)}</b><em>${t('result.mvpDamage', { damage: recap.mvp.damage })}</em></span></div><div class="recap-stats"><span><b>${recap.dealt}</b><small>${t('result.dealt')}</small></span><span><b>${recap.taken}</b><small>${t('result.taken')}</small></span><span><b>${recap.healed}</b><small>${t('result.healed')}</small></span><span><b>${recap.absorbed}</b><small>${t('result.absorbed')}</small></span><span><b>${recap.combos}</b><small>${t('result.combos')}</small></span><span><b>${recap.signatures}</b><small>${t('result.signatures')}</small></span><span class="crescendo-stat"><b>↯ ${recap.crescendos}</b><small>${t('result.crescendos')}</small></span></div></div>${contributionReport}<p class="result-moment"><strong>${t('result.moment')} :</strong> ${biggest ? `${creatureName(biggest.sourceCreatureId)} · ${biggest.amount} PV` : '—'}<br>${t('result.turns', { turns: state.turn })}</p>${contractReward}<div class="mastery-rewards">${mastery}</div>${feats}${progress}<div class="result-actions">${actionButton(`≡ ${t('result.chronicle')}`, 'result-log', 'subtle-btn')}${actionButton(win && battleSession.mode === 'gauntlet' ? t('gauntlet.again') : t('result.rematch'), 'rematch')}${win && battleSession.mode === 'ladder' && save.ladderVictories < LADDER_COUNT ? actionButton(t('result.next'), 'next-battle', 'primary-btn') : ''}${win && battleSession.mode === 'circuit' ? actionButton(t('circuit.next'), 'next-circuit', 'primary-btn') : ''}${actionButton(t('result.title'), 'title')}</div></section></div><div id="replacement-root"></div></div>`;
  const moment = screen.querySelector('.result-moment');
  if (moment) moment.innerHTML = moment.innerHTML.replace(/\bPV\b/, t('battle.hpUnit'));
  if (['ladder', 'quick', 'circuit', 'trial'].includes(battleSession.mode))
    screen
  ...
  screen.querySelector('[data-action="rematch"]').addEventListener('click', () => {
    ...
    if (battleSession.mode === 'draft') {
      startDraft();
      return;
    }
    const config = { ...battleSession };
    delete config.state;
...
function closeMoveTheater() {
  theaterTimers.forEach(clearTimeout);
  theaterTimers = [];
  clearBattleFx();
...
function runMoveTheater(moveId) {
  ...
  sound.move(move);
  const impactDelay = (save.reducedMotion ? 80 : move.signature ? 690 : 460) / save.battleSpeed,
    clearDelay = (save.reducedMotion ? 260 : move.signature ? 1600 : 1200) / save.battleSpeed;
  theaterTimers.push(
    setTimeout(() => {
...
function renderBestiary() {
  ...
  const featGallery = Object.values(FEATS)
    .map((feat) => {
      const earned = save.feats.includes(feat.id);
    ...
    .join('');
  screen.innerHTML = `<div class="shell">${topbar()}<div class="page-head"><div><span class="eyebrow">24 / 24</span><h1>${t('bestiary.title')}</h1><p>${t('bestiary.subtitle')}</p></div></div><section class="feat-hall"><div><span class="eyebrow">${save.feats.length}/${Object.keys(FEATS).length}</span><h2>${t('feat.gallery')}</h2></div><div class="feat-gallery">${featGallery}</div></section><div class="bestiary-grid">${CREATURE_IDS.map(
    (id, index) => {
      const c = CREATURES[id],
        a = AFFINITIES[c.affinity],
        unlocked = index < 6 + save.ladderVictories * 2,
        passive = PASSIVES[c.passive],
        mastery = masteryProgress(save.mastery[id] || 0),
...
function renderSettings() {
  ...
  screen.className = 'screen';
  screen.innerHTML = `<div class="shell">${topbar(previousScreen)}<div class="page-head"><div><span class="eyebrow">${t('title.help')}</span><h1>${t('settings.title')}</h1></div></div><div class="settings-grid"><section class="settings-card"><div class="toggle-row"><strong>${t('settings.language')}</strong><div class="language-buttons"><button class="subtle-btn ${i18n.lang === 'fr' ? 'active' : ''}" data-lang="fr">FR</button><button class="subtle-btn ${i18n.lang === 'en' ? 'active' : ''}" data-lang="en">EN</button></div></div><div class="toggle-row"><label for="volume"><strong>${t('settings.volume')}</strong></label><input id="volume" type="range" min="0" max="1" step=".05" value="${save.volume}"></div><div class="toggle-row"><label for="muted">${t('settings.mute')}</label><input id="muted" type="checkbox" ${save.muted ? 'checked' : ''}></div><div class="toggle-row"><label for="motion">${t('settings.motion')}</label><input id="motion" type="checkbox" ${save.reducedMotion ? 'checked' : ''}></div><div class="toggle-row"><strong>${t('settings.speed')}</strong><div class="speed-buttons"><button class="subtle-btn ${save.battleSpeed === 1 ? 'active' : ''}" data-speed="1">${t('settings.normal')}</button><button class="subtle-btn ${save.battleSpeed === 2 ? 'active' : ''}" data-speed="2">${t('settings.fast')}</button></div></div></section><section class="settings-card"><h2>${t('title.help')}</h2><p>${t('settings.controls')}</p><p class="cycle">${t('settings.affinities')}</p><button class="danger-btn" data-action="reset-save">${t('settings.reset')}</button></section></div></div>`;
  bindCommon();
  ...
  screen.querySelector('#volume').addEventListener('input', (e) => {
  ...
  screen.querySelectorAll('[data-speed]').forEach((b) =>
    b.addEventListener('click', () => {
      save.battleSpeed = Number(b.dataset.speed);
      persist();
      renderSettings();
    })
  );
...
function handleEscape() {
  ...
  if (screen.querySelector('.replacement') && !battleSession?.state.sides.player.pendingReplacement) {
    screen.querySelector('#replacement-root').innerHTML = '';
    return;
  }
...
function moveGamepadFocus(direction) {
  ...
  if (next) {
    next.focus();
    next.scrollIntoView({
      behavior: save.reducedMotion ? 'auto' : 'smooth',
      block: 'nearest',
      inline: 'nearest',
    });
    sound.ui();
...
function pollGamepad() {
  ...
  const current = pad.buttons.map((button) => Boolean(button?.pressed)),
    pressed = (index) => current[index] && !gamepadButtons[index],
    axisX = pad.axes?.[0] || 0,
    axisY = pad.axes?.[1] || 0,
    axisActive = Math.abs(axisX) > 0.62 || Math.abs(axisY) > 0.62;
  if (pressed(14) || (!gamepadAxisLatch && axisX < -0.62)) moveGamepadFocus('left');
  if (pressed(15) || (!gamepadAxisLatch && axisX > 0.62)) moveGamepadFocus('right');
  if (pressed(12) || (!gamepadAxisLatch && axisY < -0.62)) moveGamepadFocus('up');
  if (pressed(13) || (!gamepadAxisLatch && axisY > 0.62)) moveGamepadFocus('down');
  if (pressed(0)) {
    const items = gamepadFocusables();
...
document.addEventListener('keydown', (event) => {
  ...
  if (screen.dataset.page === 'battle' && !locked) {
    if (['1', '2', '3'].includes(event.key)) {
      const move = activeOf(battleSession.state, 'player').moves[Number(event.key) - 1];

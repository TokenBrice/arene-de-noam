...
function renderResults(win) {
  ...
  const mvpBlock =
    recap.mvp.damage > 0
      ? `<div class="recap-mvp"><img src="${sprite(recap.mvp.id)}" alt=""><span><small>${t('result.mvp')}</small><b>${creatureName(recap.mvp.id)}</b><em>${t('result.mvpDamage', { damage: recap.mvp.damage })}</em></span></div>`
      : '';
  ...
  const celebration = win
    ...
    : '';
  screen.innerHTML = `<div class="shell result-page results-scene results-scene--${win ? 'win' : 'loss'}${ctx.save.reducedMotion ? ' results-scene--reduced' : ''}">${topbar()}${celebration}<div class="result-shell results-stage"><section class="glass-panel result-card ${win ? 'won' : 'lost'} results-panel"><div class="result-icon results-heading">${win ? '✦' : '◇'}</div><h1 class="results-heading">${win ? t('result.victory') : t('result.defeat')}</h1><p class="results-intro">${win ? t('result.victoryText') : t('result.defeatText')}</p><div class="results-grade">${performanceHtml(ctx.pendingRewards?.grade, false, win)}</div><div class="result-actions results-reveal results-reveal--1">${actionButton(`≡ ${t('result.chronicle')}`, 'result-log', 'subtle-btn')}${actionButton(win && ctx.battleSession.mode === 'gauntlet' ? t('gauntlet.again') : t('result.rematch'), 'rematch', !win ? 'primary-btn' : 'subtle-btn')}${win && ctx.battleSession.mode === 'ladder' && ctx.save.ladderVictories < LADDER_COUNT ? actionButton(t('result.next'), 'next-battle', 'primary-btn') : ''}${win && ctx.battleSession.mode === 'circuit' ? actionButton(t('circuit.next'), 'next-circuit', 'primary-btn') : ''}${ctx.battleSession.mode === 'trial' ? actionButton(t('result.nextTrial'), 'next-trial', 'subtle-btn') : ''}${actionButton(t('result.title'), 'title')}</div><div class="results-reveal results-reveal--2">${streakReward}<div class="result-team">${team.map((c) => `<img class="${c.hp <= 0 ? 'fallen' : ''}" src="${sprite(c.id)}" alt="${creatureName(c.id)}">`).join('')}</div></div><div class="battle-recap results-reveal results-reveal--3">${mvpBlock}<div class="recap-stats"><span><b>${recap.dealt}</b><small>${t('result.dealt')}</small></span><span><b>${recap.taken}</b><small>${t('result.taken')}</small></span><span><b>${recap.healed}</b><small>${t('result.healed')}</small></span><span><b>${recap.absorbed}</b><small>${t('result.absorbed')}</small></span><span><b>${recap.combos}</b><small>${t('result.combos')}</small></span><span><b>${recap.signatures}</b><small>${t('result.signatures')}</small></span></div></div><div class="results-reveal results-reveal--4">${contributionReport}<p class="result-moment"><strong>${t('result.moment')} :</strong> ${biggest ? `${creatureName(biggest.sourceCreatureId)} · ${biggest.amount} PV` : '—'}<br>${t('result.turns', { turns: state.turn })}</p><div class="mastery-rewards">${mastery}</div>${feats}${progress}</div></section></div><div id="replacement-root"></div></div>`;
  if (!mvpBlock) screen.querySelector('.recap-stats')?.classList.add('recap-stats--solo');
  ...
  screen.querySelector('[data-action="next-trial"]')?.addEventListener('click', () => {
    ctx.selection = null;
    renderTrials();
  });
}

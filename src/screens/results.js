...
function renderResults(win) {
  ...
  const celebration = win
    ...
    : '';
  screen.innerHTML = `<div class="shell result-page">${topbar()}${celebration}<div class="result-shell"><section class="glass-panel result-card ${win ? 'won' : 'lost'}"><div class="result-icon">${win ? '✦' : '◇'}</div><h1>${win ? t('result.victory') : t('result.defeat')}</h1><p>${win ? t('result.victoryText') : t('result.defeatText')}</p>${performanceHtml(ctx.pendingRewards?.grade)}${streakReward}<div class="result-team">${team.map((c) => `<img class="${c.hp <= 0 ? 'fallen' : ''}" src="${sprite(c.id)}" alt="${creatureName(c.id)}">`).join('')}</div><div class="battle-recap"><div class="recap-mvp"><img src="${sprite(recap.mvp.id)}" alt=""><span><small>${t('result.mvp')}</small><b>${creatureName(recap.mvp.id)}</b><em>${t('result.mvpDamage', { damage: recap.mvp.damage })}</em></span></div><div class="recap-stats"><span><b>${recap.dealt}</b><small>${t('result.dealt')}</small></span><span><b>${recap.taken}</b><small>${t('result.taken')}</small></span><span><b>${recap.healed}</b><small>${t('result.healed')}</small></span><span><b>${recap.absorbed}</b><small>${t('result.absorbed')}</small></span><span><b>${recap.combos}</b><small>${t('result.combos')}</small></span><span><b>${recap.signatures}</b><small>${t('result.signatures')}</small></span><span class="crescendo-stat"><b>↯ ${recap.crescendos}</b><small>${t('result.crescendos')}</small></span></div></div>${contributionReport}<p class="result-moment"><strong>${t('result.moment')} :</strong> ${biggest ? `${creatureName(biggest.sourceCreatureId)} · ${biggest.amount} PV` : '—'}<br>${t('result.turns', { turns: state.turn })}</p>${contractReward}<div class="mastery-rewards">${mastery}</div>${feats}${progress}<div class="result-actions">${actionButton(`≡ ${t('result.chronicle')}`, 'result-log', 'subtle-btn')}${actionButton(win && ctx.battleSession.mode === 'gauntlet' ? t('gauntlet.again') : t('result.rematch'), 'rematch')}${win && ctx.battleSession.mode === 'ladder' && ctx.save.ladderVictories < LADDER_COUNT ? actionButton(t('result.next'), 'next-battle', 'primary-btn') : ''}${win && ctx.battleSession.mode === 'circuit' ? actionButton(t('circuit.next'), 'next-circuit', 'primary-btn') : ''}${actionButton(t('result.title'), 'title')}</div></section></div><div id="replacement-root"></div></div>`;

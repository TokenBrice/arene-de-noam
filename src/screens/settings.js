...
function renderSettings() {
  ...
  screen.innerHTML = `<div class="shell">${topbar(ctx.settingsReturn || 'title')}<div class="page-head"><div><span class="eyebrow">${t('title.help')}</span><h1>${t('settings.title')}</h1></div></div><div class="settings-grid"><section class="settings-card"><div class="toggle-row"><strong>${t('settings.language')}</strong><div class="language-buttons"><button class="subtle-btn ${i18n.lang === 'fr' ? 'active' : ''}" data-lang="fr" aria-pressed="${i18n.lang === 'fr'}">FR</button><button class="subtle-btn ${i18n.lang === 'en' ? 'active' : ''}" data-lang="en" aria-pressed="${i18n.lang === 'en'}">EN</button></div></div>${rangeControl('music-volume', t('settings.musicVolume'), ctx.save.musicVolume)}${rangeControl('sfx-volume', t('settings.sfxVolume'), ctx.save.sfxVolume)}<div class="toggle-row"><label for="muted">${t('settings.mute')}</label><input id="muted" type="checkbox" ${ctx.save.muted ? 'checked' : ''}></div><div class="toggle-row"><label for="motion">${t('settings.motion')}</label><input id="motion" type="checkbox" ${ctx.save.reducedMotion ? 'checked' : ''}></div><div class="toggle-row contrast-row expert-mode-row"><label for="expert-mode"><strong>${t('settings.expertMode')}</strong><small>${t('settings.expertModeHint')}</small></label><input id="expert-mode" type="checkbox" ${ctx.save.expertMode ? 'checked' : ''}></div><div class="toggle-row"><strong>${t('settings.speed')}</strong><div class="speed-buttons"><button class="subtle-btn ${ctx.save.battleSpeed === 1 ? 'active' : ''}" data-speed="1" aria-pressed="${ctx.save.battleSpeed === 1}">${t('settings.normal')}</button><button class="subtle-btn ${ctx.save.battleSpeed === 2 ? 'active' : ''}" data-speed="2" aria-pressed="${ctx.save.battleSpeed === 2}">${t('settings.fast')}</button></div></div></section><section class="settings-card"><h2>${t('title.help')}</h2><p>${t('settings.controls')}</p><p class="cycle">${t('settings.affinities')}</p><button class="danger-btn" data-action="reset-save">${t('settings.reset')}</button></section></div></div>`;
  bindCommon();

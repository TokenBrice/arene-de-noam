...
test('all affinity pairings follow the single six-affinity cycle',()=>{
  for(let a=0;a<AFFINITY_ORDER.length;a++)for(let d=0;d<AFFINITY_ORDER.length;d++){
    const expected=d===(a+1)%6?1.5:d===(a+5)%6?.75:1;
...
test('damage is deterministic and applies focus, guard, exposure, weakness, and minimum',()=>{
  const move=MOVES.lucid_arc,attacker=CREATURES.orakyn,defender=CREATURES.kordane;
  const a=calculateDamage(move,attacker,defender),b=calculateDamage(move,attacker,defender);
  assert.deepEqual(a,b);assert.equal(a.affinity,1.5);
